...
archive, no second manifest name, no in-engine download. Twelve base tile keys and 66 optional
...
- Optional **per-face overrides**: `<block>_<suffix>` keys (`stone_posx`, `leaves_negz`, …), suffix
  ...
  like any other tile. One override enumerates all 78 keys — 78 layers in the no-variant baseline,
...
Sixty-six optional keys extend the base vocabulary without touching it. A key is
...
**`12 + cell`** (12..77). `TexPack.LayerCount` is 78: the twelve base keys then the 66 override
...
soon as one override key is provided the array has all 78, unused slots holding the `missing`
...
```
texpack: <source>: N/66 per-face overrides
```
...
a directional glyph stays upright and is never mirrored. The tile vocabulary (12 base keys + 66
...
Eleven renderable `Block` enum members (Air excluded) × six faces = **66 cells**, each mapped
...
All 66 cells now resolve through the chain: exact override when the manifest provides it, else
the fallback.
...
| Heartwood | PosX | `wood_top` | 7 | `heartwood_posx` |
| Heartwood | NegX | `wood_top` | 7 | `heartwood_negx` |
| Heartwood | Top | `wood_top` | 7 | `heartwood_top` |
| Heartwood | Bottom | `wood_top` | 7 | `heartwood_bottom` |
| Heartwood | PosZ | `wood_top` | 7 | `heartwood_posz` |
| Heartwood | NegZ | `wood_top` | 7 | `heartwood_negz` |

`Heartwood` (appended after `Pumpkin`, ordinal 10) falls back to `wood_top` on all six faces: `wood_top` is the existing end-grain/cross-section wood key and is darker than `wood_side`, so a third-party pack that ships no heartwood art shows cut end-grain inside the trunk instead of re-exposing bark.

Key → cells: `stone` 6, `dirt` 6, `grass_top` 1, `grass_side` 4, `grass_bottom` 1, `sand` 12,
`wood_side` 4, `wood_top` 8, `plank` 12, `leaves` 6, `bedrock` 6, `missing` 0
(6+6+1+4+1+12+4+8+12+6+6 = 66). These are the fallback counts. Of the 66 `FaceKey` names in the
table, 63 are override-only and cover exactly one cell each; the three colliding names
...
| `tiles` | object, key → string or array of strings | no | `{}` | keys from the twelve base keys or the 66 override keys; a value is one pack-relative path or a list of them (up to 16 variants, first = primary) |
...
texpack: user://texturepacks/fancy: 1/66 per-face overrides
```
...
**Chosen: `Texture2DArray`.** Twelve images for a base-only pack, 78 once any override is used
...
4×4 atlas of the same twelve images — 9×9 when the 78 slots are in use (9×9 = 81 cells ≥ 78, so `G = 9` still holds) — and the fragment shader
maps `UV2.x` → cell rectangle (`cell = floor(vec2(mod(idx,G), floor(idx/G)))`), then samples with
`UV/G + cell/G`, with `G = 4` for a base-only pack and `G = 9` for the 78-slot array. The mesher
...
8. **Per-face overrides are all-or-nothing in VRAM.** A pack that provides even one override
   carries at least the full 78-slot array — 6.5× the base art, more with variants — because the
...
twelve base keys and the 66 `<block>_<suffix>` override keys, and `version` stays `1`: a manifest
...
- **Key order is frozen** — the twelve base keys, then the 66 face cells, never reordered. Each
...
- **A no-variant pack is byte-identical to the pre-variants build**: base key `k → layer k`,
  ...
  baseline only**.
- **`TexPack.LayerCount = 78` is the no-variant baseline slot space, not an array size.** The
...
- Hostile-pack ceiling: all 78 keys × 16 variants = **1248 layers**; at 16×16 RGBA8 (1 KiB per
  layer) that is **≈ 1248 KiB** of texture memory, plus decode time.
...
| any pack with one override, no variants | 78 | 78 × 1 | **78** (today) |
| `variants-demo` (stone 4, dirt 3, ten singles, no overrides) | 12 | 4 + 3 + 10 × 1 | **17** |
| hostile pack: all 78 keys × 16 variants | 78 | 78 × 16 | **1248** ceiling |
...
`TexPack.LayerCount = KeyCount + FaceKeys.Length = 78` (`KeyCount` stays 12) remains the
...
| 256 | 256 KiB | 3 MiB | |
| 1024 | 4 MiB | 48 MiB | |
| 4096 | 64 MiB | **768 MiB** | 78-layer override layout = **4.875 GiB** — whole-pack 4096 is not realistic; only faces that need it should pay |
